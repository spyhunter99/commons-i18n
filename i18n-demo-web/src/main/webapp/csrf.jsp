...
<%
        ...
        for (int i = 0; i < cookies3.length; i++) {
            if (cookies3[i] != null && cookies3[i].getName() != null && cookies3[i].getName().equalsIgnoreCase("locale")) {
                if (cookies3[i].getValue() != null) {
                    //TODO session.setAttribute("locale", cookies3[i].getValue());
                }
            }
        }
    }
    if (session.getAttribute("locale")==null){
        ...
        ResourceLoaderWeb.setLocale(session, ResourceLoaderWeb.detectLocale(request));
    }
    ...
    if (request.getMethod().equalsIgnoreCase("post")) {
        ...
        if ((request.getParameter("nonce") == null || request.getParameter("nonce").isEmpty())) {
           ...
           LogManager.getLogger("log").warn( "CSRF Test failed, no nonce guid." + request.getRemoteAddr() + request.getRemoteUser());
            ...
            if (noncestr == null) {
                ...
                LogManager.getLogger("log").warn( "CSRF Test failed, no session guid." + request.getRemoteAddr() + request.getRemoteUser());
                session.removeAttribute("nonce");
                response.sendRedirect("index.jsp");
            ...
            if (noncestr.equals(postedstr)) {
                ...
                LogManager.getLogger("log").debug("CSRF Test passed.");
            } else {
                //mismatch, reject it
                LogManager.getLogger("log").warn( "CSRF Test failed, session did not match nonce guid." + request.getRemoteAddr() + request.getRemoteUser());
                session.removeAttribute("nonce");
                response.sendRedirect("index.jsp");
                return;
                //throw new CrossSiteRequestForgeryException("Cross Site Request Forgery");
